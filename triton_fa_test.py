...
class TritonAttention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: Any, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, causal: bool, softmax_scale: float) -> torch.Tensor:
...
def test_op(BATCH_SIZE: int, NUM_HEADS: int, SEQ_LEN: int, HEAD_DIM: int , causal: bool, dtype=torch.float16):
    Q = (
        torch.empty(
