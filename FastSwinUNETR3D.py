...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask is not None:
            ...
            q1 = q.view(g, nw, self.num_heads, n, d).to(torch.bfloat16)
            k1 = k.view(g, nw, self.num_heads, n, d).to(torch.bfloat16)
            v1 = v.view(g, nw, self.num_heads, n, d).to(torch.bfloat16)
            with sdpa_kernel([SDPBackend.CUDNN_ATTENTION], set_priority=True):

                attn = [F.scaled_dot_product_attention(
                    q1[i], k1[i], v1[i],
                    attn_mask=mask.unsqueeze(1) + relative_position_bias.unsqueeze(0),
                    # float mask: added to scores
            ...
            attn = attn.view(b // nw, nw, self.num_heads, n, n).add_(mask.unsqueeze(0).unsqueeze(2))
...
class SwinTransformerBlockNoShift(nn.Module):
    """
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, mlp_hidden_dim), nn.GELU(), self.mlp_drop, nn.Linear(mlp_hidden_dim, dim), self.mlp_drop)
        #self.mlp = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act=act_layer, dropout_rate=drop, dropout_mode="swin")

    def forward_part1(self, x, mask_matrix):
        x_shape = x.size()
        if len(x_shape) != 5:
            raise ValueError(f"expected 5D input (got {x_shape})")
        x = self.norm1(x)
    ...
    def forward(self, x, mask_matrix):
        ...
        y = self.forward_part1(x, mask_matrix)
        torch.cuda.synchronize()
        t2 = time.perf_counter()
        print(f"Forward shortcut pls drop {t2-t1}")

        x = x + y + self.mlp(x + y)
        torch.cuda.synchronize()
        t3 = time.perf_counter()
        print(f"Forward part2: {t3 - t2}")
        print(f"Total forward: {t3-t1}")
        return x

class SwinTransformerBlockShift(nn.Module):
    """
    Swin Transformer block based on: "Liu et al.,
    Swin Transformer: Hierarchical Vision Transformer using Shifted Windows
    <https://arxiv.org/abs/2103.14030>"
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        self.blk1 = SwinTransformerBlockNoShift(
                    dim=dim,
                    num_heads=num_heads,
                    window_size=self.window_size,
        ...
        self.blk2 = SwinTransformerBlockShift(
                dim=dim,
                num_heads=num_heads,
                window_size=self.window_size,
    ...
    def forward(self, x):
        ...
        if (dp, hp, wp) in self.mask_cache:
            attn_mask = self.mask_cache[(dp, hp, wp)]
        else:
            attn_mask = compute_mask((dp, hp, wp), window_size, shift_size, x.device)
            self.mask_cache[(dp, hp, wp)] = attn_mask

        x = self.blk1(x, attn_mask)
        x = self.blk2(x, attn_mask)
        x = x.view(b, d, h, w, -1)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        print(f"Input shape: {x.shape}, {x.dtype}, {x.device}, {x.min()}, {x.max()}")
        torch.cuda.synchronize()
        t0 = time.perf_counter()
        ...
        torch.cuda.synchronize()
        t1 = time.perf_counter()
        print(f"Prepare x0: {t1 - t0}")

        x0_contig = x0.contiguous()
        torch.cuda.synchronize()
        t101 = time.perf_counter()
        print(f"Prepare x1 - x0 contig: {t101 - t1}")
        x1 = self.layers1[0](x0_contig)
        torch.cuda.synchronize()
        t102 = time.perf_counter()
        print(f"Prepare x1 - layer1: {t102 - t101}")
        x1_out = self.proj_out(x1, normalize)
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        torch.cuda.synchronize()
        t2 = time.perf_counter()
        print(f"Prepare x1 - proj: {t2 - t102}")
        print(f"Prepare x1: {t2 - t1}")
        ...
        print(f"Prepare x3: {t4 - t3}")
        x4 = self.layers4[0](x3.contiguous())
        x4_out = self.proj_out(x4, normalize)
        torch.cuda.synchronize()
        t5 = time.perf_counter()
        print(f"Prepare x4: {t5 - t4}")
        torch.cuda.synchronize()
        t6 = time.perf_counter()
        print(f"CUDA sync: {t6 - t5}")
        print(f"Total forward: {t6-t0}")
